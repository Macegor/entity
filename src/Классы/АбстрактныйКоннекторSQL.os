...
Процедура СоздатьТаблицу(ОбъектМодели) Экспорт
	
	// Получаем имя таблицы
	ИмяТаблицы = ОбъектМодели.ИмяТаблицы();
	
	// Формируем SQL запрос для создания пустой таблицы
	ШаблонСозданияТаблицы = "CREATE TABLE IF NOT EXISTS %1 ()";
	
	// Формируем итоговый запрос
	ТекстЗапроса = СтрШаблон(ШаблонСозданияТаблицы, ИмяТаблицы);
...
Процедура УдалитьТаблицу(ОбъектМодели) Экспорт
	
	// Получаем имя таблицы
	ИмяТаблицы = ОбъектМодели.ИмяТаблицы();
	
	// Формируем SQL запрос для удаления таблицы
	ШаблонУдаленияТаблицы = "DROP TABLE IF EXISTS %1";
...
Функция ПолучитьОпределениеКолонки(ОбъектМодели, Колонка) Экспорт
	
	// Получаем тип колонки
	ТипКолонкиСУБД = Коннектор.ПолучитьТипКолонкиСУБД(ОбъектМодели, Колонка);
	
	// Формируем определение колонки
	ОпределениеКолонки = Колонка.ИмяКолонки;
	
	// Добавляем тип колонки
	ОпределениеКолонки = ОпределениеКолонки + " " + ТипКолонкиСУБД;
	
	// Проверяем, является ли колонка идентификатором
	Если Колонка.ИмяПоля = ОбъектМодели.Идентификатор().ИмяПоля Тогда
		ОпределениеКолонки = ОпределениеКолонки + " PRIMARY KEY";
	КонецЕсли;
	
	// Проверяем, нужно ли автоинкрементное значение
	Если Колонка.ГенерируемоеЗначение И Не ТипКолонкиСУБД = "serial" Тогда
		ОпределениеКолонки = ОпределениеКолонки + " AUTOINCREMENT";
	КонецЕсли;
	
	Возврат ОпределениеКолонки;
	
КонецФункции

Процедура ДобавитьКолонкуВТаблицу(ОбъектМодели, Колонка) Экспорт
	
	// Получаем имя таблицы
	ИмяТаблицы = ОбъектМодели.ИмяТаблицы();
	
	// Формируем SQL запрос для добавления колонки
	ШаблонДобавленияКолонки = "ALTER TABLE %1 ADD COLUMN %2";
	
	// Получаем определение колонки
	ОпределениеКолонки = ПолучитьОпределениеКолонки(ОбъектМодели, Колонка);
	
	// Формируем итоговый запрос
	ТекстЗапроса = СтрШаблон(ШаблонДобавленияКолонки, ИмяТаблицы, ОпределениеКолонки);
	...
	// Если есть внешний ключ, добавляем его
	ОписаниеВнешнегоКлюча = Коннектор.ПолучитьОписаниеВнешнегоКлюча(ОбъектМодели, Колонка);
	Если ЗначениеЗаполнено(ОписаниеВнешнегоКлюча) Тогда
		Запрос = Коннектор.КонструкторКоннектора.НовыйЗапрос(Коннектор.Соединение);
		Запрос.Текст = ОписаниеВнешнегоКлюча;
		Запрос.ВыполнитьКоманду();
	КонецЕсли;
	
КонецПроцедуры

Процедура УдалитьКолонкуИзТаблицы(ОбъектМодели, ИмяКолонки) Экспорт
	
	// Получаем имя таблицы
	ИмяТаблицы = ОбъектМодели.ИмяТаблицы();
	
	// Формируем SQL запрос для удаления колонки
	ШаблонУдаленияКолонки = "ALTER TABLE %1 DROP COLUMN %2";
	
	// Формируем итоговый запрос
	ТекстЗапроса = СтрШаблон(ШаблонУдаленияКолонки, ИмяТаблицы, ИмяКолонки);
